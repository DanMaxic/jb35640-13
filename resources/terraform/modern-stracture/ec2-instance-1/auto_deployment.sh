...
TF_BACKEND_FILE="deployment-profiles/${DEPLOYMENT_PROFILE_NAME}/backend"
...
terraform-11.7 init -backend-config=${TF_BACKEND_FILE} -var-file=${TF_VAR_FILE}  -input=false \
    ${TF_MODULE_SRC}

if [ $? -eq 0 ]; then echo "######TERRAFORM INIT DONE######"; else echo "TERRAFORM INIT FAILED, EXITING"; exit 1; fi

terraform-11.7 plan -var-file=${TF_VAR_FILE} \
    -input=false -module-depth=-1 -detailed-exitcode \
    ${TF_MODULE_SRC}
...
if [ "$confirm" = "YES" ]; then
    ...
    terraform apply -refresh=true -auto-approve -var-file=${TF_VAR_FILE} ${TF_MODULE_SRC}
